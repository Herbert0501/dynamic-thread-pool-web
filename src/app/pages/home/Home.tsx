    ...
    <div>
      {isAuthPath || !isAuthorized ? (
        ...
        <Box sx={{ display: "flex" }}>
          ...
          <Box
            component="main"
            sx={{ flexGrow: 1, p: 3, marginTop: "3.65rem" }}
          >
            <Routes>
              <Route path={Path.Home} element={<Welcome />}></Route>
              <Route path={Path.Pools} element={<Pools />}></Route>
              <Route path={Path.Monitor} element={<Monitor />}></Route>
            </Routes>
          </Box>
        </Box>
      )}
